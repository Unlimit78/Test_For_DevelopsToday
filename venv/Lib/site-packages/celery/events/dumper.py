...
class Dumper(object):
    ...
    def on_event(self, ev):
        ...
        if type.startswith("task-"):
            ...
            if type in ("task-received", "task-sent"):
                task = TASK_NAMES[uuid] = "{0}({1}) args={2} kwargs={3}".format(
        ...
        fields = ", ".join("{0}={1}".format(key, ev[key]) for key in sorted(ev))
        sep = fields and ":" or ""
        self.say(
            "{0} [{1}] {2}{3} {4}".format(
    ...
    def format_task_event(self, hostname, timestamp, type, task, event):
        fields = ", ".join("{0}={1}".format(key, event[key]) for key in sorted(event))
        sep = fields and ":" or ""
        self.say(
            "{0} [{1}] {2}{3} {4} {5}".format(
