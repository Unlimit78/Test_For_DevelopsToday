...
class RedisBackend(BaseKeyValueStoreBackend, AsyncBackendMixin):
    ...
    def _chord_zset(self):
        transport_options = self.app.conf.get("result_backend_transport_options", {})
    ...
    def on_chord_part_return(self, request, state, result, propagate=None, **kwargs):
        ...
        with client.pipeline() as pipe:
            if self._chord_zset:
                pipeline = pipe.zadd(
                    jkey, {self.encode([1, tid, state, result]): group_index}
                ).zcount(jkey, "-inf", "+inf")
            else:
                pipeline = pipe.rpush(jkey, self.encode([1, tid, state, result])).llen(
                    jkey
                )
